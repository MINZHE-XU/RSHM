...
const path = (state=[] , action) => {
  switch (action.type) {
    ...
    let temp= needUpdate.map(function(apath, index){
      ...
      //console.log(JSON.stringify(computeBackPath(apath.path,apath.fullPath)))
      return  {
          id: apath.id,
      ...
      let temp2= needUpdate2.map(function(apath, index){
        ...
        //console.log(JSON.stringify(apath.fullPath))
        //console.log(JSON.stringify(apath.path))
        //console.log(JSON.stringify(computeBackPath(moveForward(computeBackPath(apath.path,apath.fullPath) , stepLength2) ,apath.fullPath) ))

        return  {
            id: apath.id,
            isDrone:apath.isDrone,
            path:computeBackPath(moveForward(computeBackPath(apath.path,apath.fullPath) , stepLength2) ,apath.fullPath.map(function(point, index){ return apath.fullPath[apath.fullPath.length-1-index]})),
...
export function moveForward(path,stepLength){
  if (path.length>1){
    ...
    if (distance>=stepLength){
      ...
      //console.log(aaa)
      return [firstPosition, ...path.slice(1)]
...
if(checkSame(path[0],fullPath[fullPath.length-path.length])){
  ...
  console.log(JSON.stringify(temp.map(function(point, index){ return temp[temp.length-1-index]})))
  return temp.map(function(point, index){ return temp[temp.length-1-index]})
}else{
